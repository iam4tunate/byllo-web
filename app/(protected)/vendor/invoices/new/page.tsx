    ...
    <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-16">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-primary tracking-tight">Create Invoice</h1>
            <p className="text-sm font-medium text-muted">Draft and send a new document to your client.</p>
          </div>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              {selectedClient ? (
                <div className="flex items-center justify-between bg-surface-raised border border-surface-border rounded-xl p-4 shadow-xs">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setShowClientSelector(true)} disabled={isLoading}>
                    Change
                  </Button>
                </div>
              ) : (
                ...
                >
                  ...
                  <span className="text-sm font-bold text-inherit">Click to select a client</span>
                </button>
              )}
            </div>
            ...
            <div className="space-y-4">
                      ...
                      {items.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td className="px-3 py-3 text-center">
                            <button onClick={() => removeItem(item.id)} disabled={isLoading} className="text-muted hover:text-danger p-1 rounded-md hover:bg-danger/10 transition-colors disabled:opacity-50">
                              <Trash size={16} />
                            </button>
                          </td>
                        </tr>
              ...
              <div className="flex items-end gap-3 bg-surface-raised/50 p-4 rounded-xl border border-surface-border border-dashed">
                <div className="flex-1">
                  ...
                  <input type="text" placeholder="e.g. Solar Panel" className="w-full h-10 px-3 bg-white border border-surface-border rounded-lg text-sm text-primary focus:outline-none focus:border-brand shadow-xs disabled:opacity-50"
                ...
                <div className="w-20">
                  ...
                  <input type="number" placeholder="1" min="1" className="w-full h-10 px-3 bg-white border border-surface-border rounded-lg text-sm text-primary focus:outline-none focus:border-brand shadow-xs text-right disabled:opacity-50"
                ...
                <div className="w-32">
                  ...
                  <input type="number" placeholder="0.00" className="w-full h-10 px-3 bg-white border border-surface-border rounded-lg text-sm text-primary focus:outline-none focus:border-brand shadow-xs text-right disabled:opacity-50"
                ...
                <Button 
                  ...
                  disabled={!newItem.description || newItem.quantity <= 0 || newItem.rate <= 0 || isLoading}
                >
                ...
                <DatePicker 
                   ...
                   onChange={(date) => {
                     ...
                     if (invoiceData.dueDate && date > invoiceData.dueDate) {
                       newInvoiceData.dueDate = new Date(date.getTime() + 14 * 24 * 60 * 60 * 1000);
                     }
