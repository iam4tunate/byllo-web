...
export default function VendorLayout({
  ...
  const navGroups: { title: string; items: NavItemConfig[] }[] = [
    {
      ...
      items: [
        ...
        { href: "/vendor/notifications", icon: Bell, label: "Notifications", badge: 2 },
      ]
    },
    {
      title: "Management",
      items: [
        { href: "/vendor/invoices", icon: FileText, label: "Invoices", badge: 3 },
  ...
  const renderBreadcrumbs = () => {
    ...
    crumbs.push(<span key="home" className="text-muted">Home</span>);
    
    if (parts.length > 1) {
      const p1 = parts[1]; // "dashboard", "invoices"
      const sectionName = p1.charAt(0).toUpperCase() + p1.slice(1);
      
      crumbs.push(<ChevronRight key="sep1" size={14} className="text-muted mx-1" strokeWidth={3} />);
      
      if (parts.length === 2) {
        crumbs.push(<span key="sec" className="text-primary">{sectionName}</span>);
      } else {
        const sectionSingular = sectionName.endsWith('s') ? sectionName.slice(0, -1) : sectionName;
        crumbs.push(<span key="sec" className="text-muted">{sectionSingular}</span>);
        crumbs.push(<ChevronRight key="sep2" size={14} className="text-muted mx-1" strokeWidth={3} />);
        
        const p2 = parts[2];
        const label = p2 === "new" ? "Create" : p2;
        
        if (parts.length > 3 && parts[3] === "edit") {
          crumbs.push(<span key="p2" className="text-muted">{label}</span>);
          crumbs.push(<ChevronRight key="sep3" size={14} className="text-muted mx-1" strokeWidth={3} />);
          crumbs.push(<span key="p3" className="text-primary">Edit</span>);
        } else {
          crumbs.push(<span key="p2" className="text-primary">{label}</span>);
        }
      }
    }
